...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		unsigned int victim_frame = victim_pte->frame; 
		off_t victim_swap_off = victim_pte->swap_off; 
		if (!(victim_frame & PG_DIRTY)){
			evict_clean_count++; 
		}
		else { // victim frame swapping out	
			victim_pte->frame = victim_pte->frame | PG_ONSWAP;
			evict_dirty_count++; 
		}
		victim_pte->swap_off = swap_pageout(victim_frame >> PAGE_SHIFT, victim_swap_off); 
		victim_pte->frame = (victim_pte->frame | PG_VALID) ^ PG_VALID; // set the victim_frame to invalid
	}
...
char *find_physpage(addr_t vaddr, char type) {
	// printf("in find_physpage, vaddr: %d\n", vaddr); // 100 0000 0000 0001 0000 0000 0000
    ...
    // give back the valid bit 
    uintptr_t return_valid_bit = (pgdir[idx].pde >> 1) << 1;
    page_table = (pgtbl_entry_t *) return_valid_bit;

	// Use vaddr to get index into 2nd-level page table and initialize 'p'

    p = &page_table[PGTBL_INDEX(vaddr)]; // this is the pte we are looking for 
	
	// Check if p is valid or not, on swap or not, and handle appropriately
	if (!(p->frame & PG_VALID)){ // invalid frame / no frame
		// ok, i'll give you a frame to use 
		if (!(p->frame & PG_ONSWAP)){ // frame not on swap / first time use this physical frame
		...
		else { // frame on swap, replace the victim frame 
			int frame = allocate_frame(p); 
			p->frame = frame << PAGE_SHIFT; 
			...
			if(type == 'M' || type == 'S'){
				...
				p->frame = (p->frame | PG_DIRTY) ^ PG_DIRTY;
			}
			swap_pagein(p->frame >> PAGE_SHIFT, p->swap_off);
		}	
		p->frame = p->frame | PG_VALID;// ok, set the frame to valid 
		miss_count++; // increament miss count
	}
	else { // valid frame, ok it is in the physical memory
		if(type == 'M' || type == 'S'){
			p->frame = p->frame | PG_DIRTY;
		}	
       	hit_count++; 
